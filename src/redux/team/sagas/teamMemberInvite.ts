...
import { ACTION, InviteTeamMemberAction, onSuccess, onFailure } from '../action/teamMemberInvite';
...
function* inviteTeamMember(action: InviteTeamMemberAction) {
  try {
    //-------------- Request API
    yield put(showIndicator(Colors.overlay5));
    const res: Response = yield TeamMemberAPI.inviteTeamMember(
      action.params
    );
...
export default function* saga() {
  yield takeEvery(ACTION, inviteTeamMember);
}
