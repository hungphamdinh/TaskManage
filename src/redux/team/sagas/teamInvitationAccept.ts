...
import { ACTION, AcceptTeamInvitationAction, onSuccess, onFailure } from '../action/teamInvitationAccept';
...
function* acceptTeamInvitation(action: AcceptTeamInvitationAction) {
  try {
    //-------------- Request API
    yield put(showIndicator(Colors.overlay5));
    const res: Response = yield TeamMemberAPI.acceptTeamInvitation(
      action.params
    );
...
export default function* saga() {
  yield takeEvery(ACTION, acceptTeamInvitation);
}
