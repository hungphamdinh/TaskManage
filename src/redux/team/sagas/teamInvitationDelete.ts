...
import { ACTION, DeleteInvitationAction, onSuccess, onFailure } from '../action/teamInvitationDelete';
...
function* deleteTeamInvitation(action: DeleteInvitationAction) {
  try {
    //-------------- Request API
    yield put(showIndicator(Colors.overlay5));
    const res: Response = yield TeamMemberAPI.deleteTeamInvitation(
      action.params
    );
...
export default function* saga() {
  yield takeEvery(ACTION, deleteTeamInvitation);
}
