...
import { ACTION, RejectTeamInvitationAction, onSuccess, onFailure } from '../action/teamInvitationReject';
...
function* rejectTeamInvitation(action: RejectTeamInvitationAction) {
  try {
    //-------------- Request API
    yield put(showIndicator(Colors.overlay5));
    const res: Response = yield TeamMemberAPI.rejectTeamInvitation(
      action.params
    );
...
export default function* saga() {
  yield takeEvery(ACTION, rejectTeamInvitation);
}
