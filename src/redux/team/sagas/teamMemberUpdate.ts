...
import { ACTION, UpdateTeamMemberAction, onSuccess, onFailure } from '../action/teamMemberUpdate';
...
function* updateTeamMember(action: UpdateTeamMemberAction) {
  try {
    //-------------- Request API
    console.log(action.params);
    yield put(showIndicator(Colors.overlay5));
    const res: Response = yield TeamMemberAPI.updateTeamMember(
      action.params
    );
...
export default function* saga() {
  yield takeEvery(ACTION, updateTeamMember);
}
