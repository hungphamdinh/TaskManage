...
import { ACTION, AcceptInvitationAction, onSuccess, onFailure } from '../action/invitationAccept';
...
function* acceptInvitation(action: AcceptInvitationAction) {
  try {
    //-------------- Request API
    yield put(showIndicator(Colors.overlay5));
    const res: Response = yield InvitationAPI.acceptInvitation(
      action.params
    );
...
export default function* saga() {
  yield takeEvery(ACTION, acceptInvitation);
}
