...
import { onSuccess, onFailure, ACTION, LeaveTaskAction } from '../action/taskLeave';

function* leaveTask(action: LeaveTaskAction) {
  try {
    //-------------- Request API
    // console.log(action)
    yield put(showIndicator(Colors.overlay5));
    yield sleep(1000);
    const res = yield TaskAPI.leaveTask(action.params);
    // console.log(res);
    //-------------- Request API Success
...
export default function* saga() {
  yield takeEvery(ACTION, leaveTask);
}
