...
import { ACTION_UPDATE_USER_PROFILE, UpdateUserProfileAction } from '../action/user';
...
function* updateUserProfile(action: UpdateUserProfileAction) {
  try {
    ...
    yield sleep(1000);
    const res: Response = yield UserAPI.updateUserProfile(
      action.params
    );
    //-------------- Request API Success
...
export default function* saga() {
  yield takeEvery(ACTION_UPDATE_USER_PROFILE, updateUserProfile);
}
